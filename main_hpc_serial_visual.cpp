...
// Function to save the grid state to a binary file
void save_grid_state(const SimulationData& sim, int step) {
    std::string filename = "output_step_" + std::to_string(step) + ".bin";
    std::ofstream outfile(filename, std::ios::binary);

    // Dimensions
    outfile.write(reinterpret_cast<const char*>(&sim.width), sizeof(int));
    outfile.write(reinterpret_cast<const char*>(&sim.height), sizeof(int));
    
    // State vector
    outfile.write(reinterpret_cast<const char*>(sim.state.data()), sim.state.size() * sizeof(uint8_t));
    
    // Population density
    outfile.write(reinterpret_cast<const char*>(sim.popDensity.data()), sim.popDensity.size() * sizeof(float));
...
int main(int argc, char** argv) {
    ...
    save_grid_state(sim, 0); // Save initial state
